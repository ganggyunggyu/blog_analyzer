from _prompts.constants import REWRITE_MEMO_PROMPT
...
아래 요소를 반드시 포함하여, 특정 제품·서비스·경험을 주제로 원고를 작성하라.
제품이 무엇인지와 관계없이 이 구조와 요소를 이행해야 한다.

1) **제품에 대한 상세한 설명 및 관련 활용 법**
   - 제품의 특징·성분·구성 요소·안전 인증·제조 과정 등 객관적 정보 포함.
   - “어떻게 활용했는지”, “섭취/사용 방법”, “운동·식단·환경과 병행한 사례” 등 실제 사용법을 구체적으로 적는다.
   - 자연스러운 문장으로 이어가되, 세부 설명(성분, 인증, 활용 순서)을 빠짐없이 넣는다.

2) **활용해보니 생긴 효과와 상세한 수치 변화 및 주변의 반응**
   - 제품/서비스 사용 후 느낀 변화, 효과를 서술.
   - 가능하다면 수치·퍼센트·전후 비교 등 정량적 데이터를 넣는다.
   - 외형·건강·환경 변화, 주변 사람의 반응·칭찬·놀람 같은 요소도 포함.
   - 결과에 대한 개인의 소감과 만족도, 예상치 못한 장점/아쉬움도 언급.

3) **비슷한 카테고리의 제품 및 키워드를 탐색한 후에(예: 다이어트 보조제라면? 위고비, 마운자로 등)에서 보고된 부작용 및 단점**
   - 관련 상품을 직접 인터넷을 탐색해서 찾아와야해 그 상품과 키워드의 비교 그리고 긍정적인 면 부정적인 면에대해 사용자가 궁금해할만한 정보를 확실히 전달해
   - 관련 제품 및 관련 키워드에서 언급되는 부작용(예: 위고비 및 마운자로라면? 메스꺼움, 구토, 위장 불편감, 복부 팽만, 두통, 피로감 등)과 단점을 객관적으로 서술.
   - 부작용이 나타날 가능성, 개인차, 관리 방법, 전문가 상담 필요성을 함께 언급.
   - 해당 약품을 직접 권유하거나 판매하지 않고, {keyword}와의 차이 또는 주의사항을 설명하는 맥락으로 제시.

글은 1인칭 화자의 자연스러운 스토리텔링을 중심으로, 정보와 경험이 균형 있게 드러나야 한다.

1️⃣ 문제 인식과 동기  
- 일상이나 환경에서 개선이 필요하다고 느낀 지점을 명확히 제시한다.  
- 해당 문제를 해결하거나 상태를 향상시키고자 한 계기·목표를 설명한다.

2️⃣ 선택과 의사결정 과정  
- 문제 해결을 위해 먼저 시도한 일반적 방법이나 기존 접근 방식을 공유한다.  
- 기대만큼 성과가 없거나 더 나은 효과를 원해 특정 솔루션·프로그램·전략을 채택하게 된 배경을 서술한다.

3️⃣ 실행 및 관리 요소  
- 선택한 해결책의 비용, 준비 과정, 운영 방식, 소요 기간을 투명하게 소개한다.  
- 실행 중 필요한 관리 포인트나 현실적인 제약을 함께 설명한다.

4️⃣ 결과와 성취  
- 일정 기간 실행 후 측정 가능한 변화(수치, 전후 비교, 구체적 개선점)를 제시한다.  
- 그에 따른 만족감, 배운 점, 긍정적 변화를 서술한다.

5️⃣ 리스크와 교훈  
- 과정에서 발견된 예상 밖의 어려움·부작용·위험 요인을 객관적으로 언급한다.  
- 비슷한 상황의 사람들에게 주의할 점이나 전문가 조언 필요성을 제안한다.

[작성 규칙]  
- 화자는 1인칭으로 자연스럽게 이야기하며, 경험과 정보를 동시에 전달해야 한다.  
- 과장·광고성 표현은 지양하고, 솔직한 후기와 사실 기반 서술을 유지한다.  
- 문장은 간결하되 부드럽게 연결하며, 독자가 흐름을 쉽게 따라갈 수 있도록 구성한다.  
- 주제와 상관없이 위 다섯 가지 요소를 모두 포함해 글을 완성한다.
...
다음 파이프라인을 실행해 "{keyword}" 최신 블로그 키워드를 자동 반영한 원고를 작성해.
...
네이버 검색해서 키워드에 대한 어떤 정보를 얻었는지 확실히 결과 리턴을 같이 해야해 ---으로 구분지어
참고한 블로그 링크를 줘도돼
