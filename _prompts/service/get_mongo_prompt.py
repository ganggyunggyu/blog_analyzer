...
- 소제목은 5~7개 사이로 유동적으로 조절해서 사용하세요.
...
[템플릿 예시]
- 템플릿의 전체 구조·문장 흐름은 최대한 유지하세요.
- 다만 어미, 화자의 말투, 연결 어감을 약간만 바꿔 자연스럽게 만드세요.
- 템플릿 안의 대괄호([ ])는 변수 표시용이니 그대로 쓰지 말고 알맞은 단어로 교체하세요.
- 제품 브랜드명, 작성자 이름 등은 그대로 쓰지 말고 가상의 이름으로 바꾸세요.
- 이모지, ㅎㅎ, ㅋㅋ, ㅠㅠ, !! 같은 감정 표현은 템플릿처럼 자연스럽게 포함하세요.
