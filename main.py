...
def run_template_generation(directory_path: str) -> List[Dict[str, str]]:
    ...
    for fp in tqdm(files, desc="템플릿 생성 중", unit="파일"):
        ...
        for sent in split_sentences(content):
            has_param = any(
                any(v in sent for v in vals) for _, vals in parameters.items()
            )
            if not has_param:
...
def run_subtitle_extraction(directory_path: str) -> Dict[str, List[str]]:
    """
    ...
    for fp in tqdm(files, desc="부제목 추출 중", unit="파일"):
        ...
        if not content.strip():
            ...
            continue
        try:
            subs = extract_subtitles_with_ai(full_text=content)
...
def save_templates_to_mongo(templates: List[Dict[str, str]]) -> None:
    ...
    now = datetime.utcnow()
    docs = [
        {"timestamp": now, "templated_text": t["templated_text"]} for t in templates
    ]
    ...
    try:
        ...
        if unique_words:
            db.insert_many_documents(
                "morphemes", [{"timestamp": ts, "word": w} for w in unique_words]
            )
        ...
        if sentences:
            db.insert_many_documents(
                "sentences", [{"timestamp": ts, "sentence": s} for s in sentences]
            )
        ...
        save_subtitles(run_subtitle_extraction(directory_path), ts)
    ...
    while True:
        ...
        try:
            ...
            if choice in ("1", "morpheme"):
                ...
                if not (unique_words and sentences and expressions and parameters):
                    click.echo(
                        "DB에 원고 생성을 위한 데이터가 부족합니다. 먼저 분석 저장을 실행하세요."
                    )
                else:
                    user_instructions = click.prompt(
                        "원고 작성 추가 지시사항 (빈칸 가능)", type=str, default=""
                    )
                    # 간단 스피너
