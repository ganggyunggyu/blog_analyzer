import time
from datetime import datetime
from fastapi import HTTPException, APIRouter
from fastapi.concurrency import run_in_threadpool

from mongodb_service import MongoDBService
from utils.get_category_db_name import get_category_db_name
from schema.generate import GenerateRequest
from llm.gemini_3_flash_clean_service import gemini_3_flash_clean_gen, MODEL_NAME
from utils.query_parser import parse_query
from utils.progress_logger import progress


router = APIRouter()


@router.post("/generate/gemini-3-flash-clean")
async def generator_gemini_3_flash_clean(request: GenerateRequest):
    """
    Gemini 3 Flash Preview 클린 버전 (프롬프트 없음)
    """
    start_ts = time.time()
    service = request.service.lower()
    keyword = request.keyword.strip()
    ref = request.ref

    c_elapsed = time.time() - start_ts

    kw_clean = keyword.replace("\n", " ").replace("  ", " ").strip()
    kw_display = kw_clean[:50] + "..." if len(kw_clean) > 50 else kw_clean

    print("\n" + "=" * 60)
    print(f"🚀 GEMINI-3-FLASH-CLEAN 원고 생성 시작")
    print("=" * 60)
    print(f"📌 서비스    : {service.upper()}")
    print(f"🎯 키워드    : {kw_display}")
    print(f"🤖 모델      : {MODEL_NAME}")
    print(f"📝 참조원고  : {'✅ 있음' if len(ref) != 0 else '❌ 없음'}")
    print(f"⏱️  분류시간  : {c_elapsed:.2f}s")
    print("=" * 60 + "\n")

    db_service = MongoDBService()

    is_ref = len(ref) != 0

    try:
        with progress(label=f"{service}:{MODEL_NAME}:{kw_display}"):
            generated_manuscript = await run_in_threadpool(
                gemini_3_flash_clean_gen,
                user_instructions=keyword,
                ref=ref,
            )

        if generated_manuscript:

            parsed = parse_query(keyword)

            document = {
                "content": generated_manuscript,
                "createdAt": datetime.now(),
                "engine": MODEL_NAME,
                "service": f"{service}_gemini_3_flash_clean",
                "keyword": keyword,
                "ref": ref if ref else "",
            }

            try:
                db_service.insert_document("manuscripts", document)

                if is_ref:
                    ref_document = {"content": ref, "keyword": parsed["keyword"]}
                    db_service.insert_document("ref", ref_document)

                document["_id"] = str(document["_id"])
                elapsed = time.time() - start_ts

                print("\n" + "=" * 60)
                print(f"✅ GEMINI-3-FLASH-CLEAN 원고 생성 완료")
                print("=" * 60)
                print(f"🎯 키워드       : {kw_display}")
                print(f"⏱️  총 소요시간  : {elapsed:.2f}s")
                print(f"💾 DB 저장      : ✅ 성공")
                print("=" * 60 + "\n")

                return document
            except Exception as e:
                print(f"GEMINI-3-FLASH-CLEAN 데이터베이스에 저장 실패: {e}")
        else:
            raise HTTPException(
                status_code=500,
                detail="GEMINI-3-FLASH-CLEAN 원고 생성에 실패했습니다.",
            )
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"GEMINI-3-FLASH-CLEAN 원고 생성 중 오류 발생: {e}"
        )
    finally:
        if db_service:
            db_service.close_connection()
