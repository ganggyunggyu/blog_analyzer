...
from llm.gpt4o_service import gpt4o_gen, MODEL_NAME
...
@router.post("/generate/gpt4o")
async def generate_gpt4o(request: GenerateRequest):
    """
    GPT-4O 텍스트 생성기
    """
    start_ts = time.time()
    ...
    try:
        with progress(label=f"{service}:{MODEL_NAME}:{keyword}"):
            generated_manuscript = await run_in_threadpool(
                gpt4o_gen, user_instructions=keyword, ref=ref, category=category
            )
        ...
        if generated_manuscript:
            ...
            try:
                ...
                log.divider()
                log.success("GPT-4O 완료", 키워드=keyword, 길이=f"{char_count}자", 시간=f"{elapsed:.1f}s")
            ...
            raise HTTPException(
                status_code=500,
                detail="GPT-4O 원고 생성에 실패했습니다. 내부 로그를 확인하세요.",
            )
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"GPT-4O 원고 생성 중 오류 발생: {e}")
    finally:
        if db_service:
