...
from llm.requirement_analysis_service import requirement_analysis_gen, model_name
...
@router.post("/generate/requirement-analysis")
async def generator_requirement_analysis(request: GenerateRequest):
    """
    업체 요구사항 분석 서비스
    """
    start_ts = time.time()
    ...
    try:
        with progress(label=f"{service}:{model_name}:{keyword}"):
            generated_manuscript = await run_in_threadpool(
                requirement_analysis_gen, user_instructions=keyword, ref=ref, category=category
            )
        ...
        if generated_manuscript:
            ...
            try:
                ...
                print(f"업체 요구사항 분석 데이터베이스에 저장 실패: {e}")
        else:
            raise HTTPException(
                status_code=500,
                detail="업체 요구사항 분석 생성에 실패했습니다. 내부 로그를 확인하세요.",
            )
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"업체 요구사항 분석 생성 중 오류 발생: {e}")
    finally:
        if db_service:
