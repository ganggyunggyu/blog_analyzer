...
from llm.requirement_analysis_service import requirement_analysis_gen
...
async def generator_requirement_analysis(request: GenerateRequest):
    ...
    try:
        with progress(label=f"{service}:{keyword}"):
            generated_manuscript = await run_in_threadpool(
                requirement_analysis_gen,
        ...
        raise HTTPException(
            status_code=500, detail=f"업체 요구사항 분석 생성 중 오류 발생: {e}"
        )
    finally:
        if db_service:
