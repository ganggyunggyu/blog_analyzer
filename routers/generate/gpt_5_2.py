...
from llm.gpt_5_2_service import gpt_5_2_gen, model_name
...
@router.post("/generate/gpt-5-2")
async def generator_gpt_5_2(request: GenerateRequest):
    """
    ...
    try:
        with progress(label=f"{service}:{model_name}:{keyword}"):
            generated_manuscript = await gpt_5_2_gen(user_instructions=keyword, ref=ref)
