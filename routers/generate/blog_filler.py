...
from llm.blog_filler_service import blog_filler_gen, MODEL_NAME
...
async def generator_blog_filler(request: GenerateRequest):
    """블로그 글밥 쌓기 전용 생성기 (DeepSeek V4 Flash)"""
    start_ts = time.time()
    ...
    try:
        with progress(label=f"{service}:{MODEL_NAME}:{keyword}"):
            generated_manuscript = await run_in_threadpool(
                blog_filler_gen, user_instructions=keyword, ref=ref, category=category
            )
        ...
        if generated_manuscript:
            ...
            document = {
                ...
                "engine": MODEL_NAME,
                "service": f"{service}_blog_filler",
                "category": category,
                "keyword": keyword,
            }
