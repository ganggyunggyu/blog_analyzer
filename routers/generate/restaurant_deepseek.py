...
from llm.restaurant_deepseek_service import restaurant_deepseek_gen, MODEL_NAME
...
class GenerateRequest(BaseModel):
    service: str = "restaurant-deepseek"
    keyword: str
    ref: str = ""


@router.post("/generate/restaurant-deepseek")
async def generate_restaurant_deepseek(request: GenerateRequest):
    """Restaurant DeepSeek으로 원고 생성"""
    service = (request.service or "restaurant-deepseek").lower()
    ...
    try:
        ...
        if generated_manuscript:
            ...
            raise HTTPException(
                status_code=500,
                detail="원고 생성에 실패했습니다. DeepSeek 모델 응답을 확인해주세요.",
            )
