...
from llm.deepseek_service import deepseek_gen, MODEL_NAME
...
async def generator_deepseek(request: GenerateRequest):
    """
    DeepSeek 텍스트 생성기
    """
    start_ts = time.time()
    ...
    try:
        with progress(label=f"{service}:{MODEL_NAME}:{keyword}"):
            generated_manuscript = await run_in_threadpool(
                deepseek_gen, user_instructions=keyword, ref=ref, category=category
            )
        ...
        if generated_manuscript:
            ...
            try:
                ...
                print(f"DeepSeek 데이터베이스에 저장 실패: {e}")
        else:
            raise HTTPException(
                status_code=500,
                detail="DeepSeek 원고 생성에 실패했습니다. 내부 로그를 확인하세요.",
            )
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"DeepSeek 원고 생성 중 오류 발생: {e}")
    finally:
        if db_service:
