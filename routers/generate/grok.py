...
from llm.grok_service import grok_gen, MODEL_NAME
...
@router.post("/generate/grok")
async def generator_grok(request: GenerateRequest):
    """
    Grok 텍스트 생성기
    """
    start_ts = time.time()
    ...
    try:
        with progress(label=f"{service}:{MODEL_NAME}:{keyword}"):
            generated_manuscript = await run_in_threadpool(
                grok_gen, user_instructions=keyword, ref=ref, category=category
            )
        ...
        if generated_manuscript:
            ...
            try:
                ...
                log.divider()
                log.success("Grok 완료", 키워드=keyword, 카테고리=category, 시간=f"{elapsed:.1f}s")
            ...
            raise HTTPException(
                status_code=500,
                detail="Grok 원고 생성에 실패했습니다. 내부 로그를 확인하세요.",
            )
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Grok 원고 생성 중 오류 발생: {e}")
    finally:
        if db_service:
