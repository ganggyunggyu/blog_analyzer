...
from llm.review_service import review_gen, model_name
...
async def generator_review(request: GenerateRequest):
    start_ts = time.time()
    ...
    try:
        with progress(label=f"{service}:{model_name}:{keyword}"):
            generated_manuscript = await run_in_threadpool(
                review_gen, user_instructions=keyword, ref=ref, category=category
            )
        ...
        if generated_manuscript:
            ...
            try:
                ...
                print(f"독후감 비평 데이터베이스에 저장 실패: {e}")
        else:
            raise HTTPException(
                status_code=500,
                detail="독후감 비평 원고 생성에 실패했습니다. 내부 로그를 확인하세요.",
            )
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"독후감 비평 원고 생성 중 오류 발생: {e}")
    finally:
        if db_service:
