...
from llm.grok_new_service import grok_new_gen, MODEL_NAME
...
@router.post("/generate/grok-new")
async def generator_grok_new(request: GenerateRequest):
    """Grok New 범용 정보성 원고 생성기"""
    start_ts = time.time()
    ...
    try:
        with progress(label=f"{service}:{MODEL_NAME}:{keyword}"):
            generated_manuscript = await run_in_threadpool(
                grok_new_gen, user_instructions=keyword, ref=ref, category=category
            )
