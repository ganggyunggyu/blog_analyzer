"""Grok 한려담원 라우터 - 기존 Grok 기반 + 한려담원 자연스러운 추천"""
...
from llm.grok_hanryeo_service import grok_hanryeo_gen, MODEL_NAME
...
@router.post("/generate/grok-hanryeo")
async def generator_grok_hanryeo(request: GenerateRequest):
    """Grok 한려담원 원고 생성기 - 키워드 정보 + 한려담원 자연스러운 추천"""
    start_ts = time.time()
    ...
    try:
        with progress(label=f"{service}:{MODEL_NAME}:{keyword}"):
            generated_manuscript = await run_in_threadpool(
                grok_hanryeo_gen, user_instructions=keyword, ref=ref, category=category
            )
        ...
        if generated_manuscript:
            ...
            document = {
                ...
                "engine": MODEL_NAME,
                "service": f"{service}_grok_hanryeo",
                "category": category,
                "keyword": keyword,
                "ref": ref if ref else "",
            }
            ...
            raise HTTPException(
                status_code=500,
                detail="Grok 한려담원 원고 생성에 실패했습니다.",
            )
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Grok 한려담원 원고 생성 중 오류 발생: {e}")
    finally:
        if db_service:
