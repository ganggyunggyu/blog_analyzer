...
from llm.clean_deepseek_service import clean_deepseek_gen, MODEL_NAME
...
class GenerateRequest(BaseModel):
    service: str = "clean-deepseek"
    keyword: str
    ref: str = ""


@router.post("/generate/clean-deepseek")
async def generate_clean_deepseek(request: GenerateRequest):
    """프롬프트 없이 DeepSeek으로 생성"""
    service = (request.service or "clean-deepseek").lower()
    ...
    try:
        ...
        if generated_manuscript:
            ...
            raise HTTPException(
                status_code=500,
                detail="원고 생성에 실패했습니다. Clean DeepSeek 모델 응답을 확인해주세요.",
            )
