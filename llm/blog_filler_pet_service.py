...
- 문장: 짧고 간결하게 (15~30자), 한 문장에 하나의 정보
- 문단: 2~4문장, 문단 사이 빈 줄로 여백 확보
- 1인칭 표현 자연스럽게 활용 ("저도", "제가 키워보니")

[필수 규칙]
- 글자수: 공백 제외 2,000~3,000자
...
주제: 비숑프리제 분양 가이드
메인 키워드: 비숑프리제 분양
연관 키워드: 비숑 성격, 비숑 털빠짐, 미니비숑, 비숑 분양가
필명: 초파맘
타겟 독자: 소형견 입양을 고민하는 초보 보호자
강조할 포인트: 털 관리 난이도, 분리불안 주의, 분양가 현실

[예시 2]
주제: 스코티시폴드 입양 전 알아야 할 것
메인 키워드: 스코티시폴드 분양
연관 키워드: 스코티시폴드 유전병, 스코티시폴드 성격, 하이랜드폴드
필명: 고양이집사
타겟 독자: 고양이 입양을 처음 고민하는 예비 집사
강조할 포인트: 골연골이형성증 유전병, 관리 포인트
"""
...
def blog_filler_pet_gen(
    ...
    user = USER_PROMPT.format(keyword=keyword, category=category, note=note, ref=ref)
