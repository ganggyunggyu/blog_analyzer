...
- `ruff` 설정이 있으면 lint gate 는 `uv run ruff check` 를 우선 사용함.
- 타입 검증이 가능하면 `uv run mypy` 를 사용하고, 전체 레포 기준으로 과도하게 깨져 있으면 변경 파일 대상 검증으로 범위를 명시함.
- 기본 종료 게이트는 변경 파일 기준 `uv run ruff check` 후 관련 `uv run pytest` 순서로 실행함.
- unit 통과만으로 부족하면 import smoke, 라우터 호출, 재현 스크립트 중 하나로 원하는 결과까지 확인함.
