...
class BlogDataCollector:
    ...
    def collect_database_data(self, db_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 추가 메타데이터
            collection_stats = {}
            collections = ["morphemes", "sentences", "expressions", "parameters", "subtitles", "templates"]
    ...
    def format_for_claude(self, all_data: Dict[str, Any]) -> str:
        ...
        output_lines.append(f"수집 시간: {collection_info.get('collected_at', 'Unknown')}")
        output_lines.append(f"MongoDB 연결: {'성공' if collection_info.get('mongodb_available') else '실패 (데모 데이터)'}")
